...
class ModuleExec:
    ...
    def __init__(self, handle, module_hash, device, meta, block_dim: int, lowered_names=None):
        self.handle = handle
            ...
            if device.is_cpu:
                ...
                if cuda_module is not None:
                    ...
                    module_exec = ModuleExec(
                        cuda_module, module_hash, device, meta, active_block_dim, lowered_names=lowered_names
                    )
...
def launch(
    ...
    if total_dim_size > 0:
        ...
        if specialize_kernel and device.is_cuda and not adjoint and not record_cmd:
            ...
            # Spec runs correctly outside graph capture but loses the
            # replay amortization that makes it worthwhile.  Per-launch
            # JIT + CPU dispatch overhead dominates; eager mode is
            # roughly 47% slower than the non-spec baseline on typical
            # workloads.  Wrap launches in ``wp.ScopedCapture`` to
            # amortize.  Spec still fires here — silently falling back
            # would break debuggability (spec codegen issues couldn't be
            # reproduced without capture).  ``once=True`` deduplicates
            # the warning so a hot-loop launch doesn't spam.
            if capture_graph is None:
                log_warning(
                    "wp.launch is taking the spec path outside graph capture.  "
                    "Spec is correct here but ~47% slower than baseline due to per-launch "

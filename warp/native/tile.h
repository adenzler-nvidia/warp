...
private:
    ...
    template <size_t... Is>
    inline CUDA_CALLABLE int64_t _byte_offset_from_coord(const Coord& coord, wp::index_sequence<Is...>) const
    {
        return ((int64_t(strides_at<Is>(data)) * (offset[Is] + coord[Is])) + ...);
    }

    template <size_t... Is>
    inline CUDA_CALLABLE bool _checked_byte_offset(const Coord& coord, int64_t& result, wp::index_sequence<Is...>) const
    {
        int64_t idx = 0;
        bool ok = (((offset[Is] + coord[Is]) < shape_at<Is>(data)
                    ? (idx += int64_t(strides_at<Is>(data)) * (offset[Is] + coord[Is]), true)
                    : false) && ...);
        result = idx;
        return ok;
    }
...
public:
    ...
    {
        return _byte_offset_from_coord(coord, wp::make_index_sequence<Shape::N>{});
    }
    ...
    inline CUDA_CALLABLE bool index(const Coord& coord, int64_t& out) const
    {
        if constexpr (BoundsCheck) {
            int64_t byte_off;
            if (!_checked_byte_offset(coord, byte_off, wp::make_index_sequence<Shape::N>{}))
                return false;
        ...
        // transposed shared tiles, and partial tiles with OOB skipping.
        {
            ...
            if constexpr (Shape::N == 1) {
                ...
                if (WP_TILE_THREAD_IDX < Layout::Size)
                    iter.init(Layout::coord_from_linear(WP_TILE_THREAD_IDX), dest.data.strides.dims, dest.offset.indices);
    ...
    template <typename Global> inline CUDA_CALLABLE void copy_from_global(const Global& src)
    {
        ...
        // partial tiles with out-of-bounds zero-padding.
        {
            ...
            if constexpr (Shape::N == 1) {
                ...
                Iter iter;
                if (WP_TILE_THREAD_IDX < Layout::Size)
                    iter.init(Layout::coord_from_linear(WP_TILE_THREAD_IDX), src.data.strides.dims, src.offset.indices);
...
inline CUDA_CALLABLE bool compute_index_step(
    const ArrT& src, IndicesTile& indices, int axis, Coord offset, Coord c, int64_t& index)
{
    int g;
    if (I == axis) {
...
compute_index(ArrT& src, IndicesTile& indices, int axis, Coord offset, Coord c, int64_t& out)
{
...
inline CUDA_CALLABLE void tile_store(ArrT& dest, int x, Tile& src)
{
    src.copy_to_global(tile_global_t<T, typename Tile::Layout::Shape, BoundsCheck, Aligned, ArrT>(dest, tile_coord(x)));
}
template <typename T, bool BoundsCheck, bool Aligned, typename Tile, typename ArrT>
inline CUDA_CALLABLE void tile_store(ArrT& dest, int x, int y, Tile& src)
{
    src.copy_to_global(
        tile_global_t<T, typename Tile::Layout::Shape, BoundsCheck, Aligned, ArrT>(dest, tile_coord(x, y))
    );
}
template <typename T, bool BoundsCheck, bool Aligned, typename Tile, typename ArrT>
inline CUDA_CALLABLE void tile_store(ArrT& dest, int x, int y, int z, Tile& src)
{
    src.copy_to_global(
        tile_global_t<T, typename Tile::Layout::Shape, BoundsCheck, Aligned, ArrT>(dest, tile_coord(x, y, z))
    );
}
template <typename T, bool BoundsCheck, bool Aligned, typename Tile, typename ArrT>
inline CUDA_CALLABLE void tile_store(ArrT& dest, int x, int y, int z, int w, Tile& src)
{
    src.copy_to_global(
        tile_global_t<T, typename Tile::Layout::Shape, BoundsCheck, Aligned, ArrT>(dest, tile_coord(x, y, z, w))
    );
}

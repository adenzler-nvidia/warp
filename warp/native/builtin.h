...
// Scalar atomic_sub via atomicAdd of negated value.  CUDA hardware
// atomicSub is only available for `int` and `unsigned int` so we
// don't gain anything by switching on type — atomicAdd(addr, -val)
// produces the same result for every supported numeric type and is
// already what the array-based `atomic_sub(const A<T>&, ...)`
// overloads in array.h expand to.  Required so the scheme-B
// `wp_atomic_sub_baked_<N>d<...>` helper (generated by
// `register_specialized_builtin`) can call `wp::atomic_sub(T*, T)`
// — without this overload, every kernel that uses `atomic_sub` on a
// baked array silently fails to specialize and runs unbaked.
template <typename T> inline CUDA_CALLABLE T atomic_sub(T* buf, T value) { return atomic_add(buf, T(-value)); }

...
// Pointer-level ``atomic_sub`` via ``atomicAdd`` of the negated
// value.  CUDA hardware ``atomicSub`` only exists for ``int`` /
// ``unsigned int``, so switching on type gains nothing.  The
// ``ArrayLike`` ``atomic_sub`` overloads in array.h forward here.
